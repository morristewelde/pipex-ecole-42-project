...
/*   Updated: 2024/12/03 23:46:44 by mtewelde         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	pipex->filein = open(av[1], O_RDONLY, 0644);
	if (pipex->filein < 0)
	{
		free(pipex);
...
void	pipe_init(char **av, char **envp, t_pipex *pipex)
{
	...
	pipex->pid1 = fork();
	if (pipex->pid1 == -1)
		ft_error("System Fork Error on child 1");
	if (pipex->pid1 == 0)
		child_p1(av, pipex, envp);
	pipex->pid2 = fork();
	if (pipex->pid2 == -1)
		ft_error("System Fork Error on chlid 2");
	if (pipex->pid2 == 0)
